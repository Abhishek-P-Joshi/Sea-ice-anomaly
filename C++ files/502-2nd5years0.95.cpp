// 502lag1.cpp : Defines the entry point for the console application.
//
...
#define x 63
#define y 63
int t = 572, landmissingcount = 0;//x-number of rows, y-number of columns, t-no of weeks. Right now, initialized to small data set.
int degree[63][63];
int node[63][63];
int node1d[3969];
double totalcluster = 0, totalclusterrandom = 0;
...
int main()
{
	...
	for (i = 0; i < x; i++)
	{
		for (j = 0; j < y; j++)
		{
			...
			if (landmissingcount != (t-312))
				mean[i][j] = sum / ((t-312) - landmissingcount);
			//printing
				...
				for (n = 0; n < y; n++)
				{
					...
					for (k = 312; k < t; k++)
					{
						if (!(data1[i][j][k] == 168.00 || data1[i][j][k] == 157.00 || data1[m][n][k] == 168.00 || data1[m][n][k] == 157.00))
							Sxy = Sxy + ((data1[i][j][k] - mean[i][j])*(data1[m][n][k] - mean[m][n])); //To find Sxy temporarily for 2 cells
					}
					...
					if ((abs(corelation) >= 0.95) && !((i == m) && (j == n)))
					{
	...
	for (i = 0; i < x; i++)
		for (j = 0; j < y; j++)
		{
			...
			//cout << "\nSum of clustering coeff = " << totalcluster;
	...
	totalclusterrandom = avgdegree / nodecount;
	...
	degreewrite.open("degreefor095scnd5y.txt", ios::app | ios::out);
