...
int main()
{
		...
		for (j = 0; j < y; j++)
			for (m = 0; m < x; m++)
				for (n = 0; n < y; n++)
				{
					...
					for (k = 0; k+1 < t; k++)
					{
						if (!(data1[i][j][k] == 168.00 || data1[i][j][k] == 157.00 || data1[m][n][k+1] == 168.00 || data1[m][n][k+1] == 157.00))
							Sxy = Sxy + ((data1[i][j][k]-mean[i][j])*(data1[m][n][k+1])-mean[i][j]); //To find Sxy temporarily for 2 cells
					}
					...
					if ((abs(corelation) >= 0.9) && !((i == m) && (j == n))&& (edge[m][n][i][j]==0)&&(edge[i][j][m][n]==0))
					{
  ...
  double totalclusterrandom = avgdegree / nodecount;
	...
	degreewrite.open("degreefor09lag1week.txt", ios::app | ios::out);
